...
class PaymentSection(ft.Container):
    def __init__(self):
        ...
        self.products_grid = ft.GridView(
            ...
            run_spacing=AppSpacing.SMALL,
        )
        
        self.cart_list = ft.ListView(expand=True, spacing=AppSpacing.SMALL)
        self.total_text = ft.Text("Total: $0.00", style=AppTextStyles.HEADER_MEDIUM)
        self.dialog_total_text = ft.Text("Total: $0.00", style=AppTextStyles.HEADER_MEDIUM) # Separate control for dialog

        self.pay_button = ft.ElevatedButton(
            "Pay", 
            icon=ft.Icons.PAYMENT, 
            style=ft.ButtonStyle(bgcolor=AppColors.SUCCESS, color=AppColors.TEXT_INVERSE),
        ...
        self.payment_method_dropdown = ft.Dropdown(
            ...
            options=[
                ft.dropdown.Option(pm.value, pm.name.replace("_", " ").title())
                for pm in PaymentMethod
            ],
            value=PaymentMethod.CASH.value
        )

        self.change_text = ft.Text("Change: $0.00", style=AppTextStyles.HEADER_SMALL)
        ...
        self.content = ft.Row(
            [
                ...
                ft.Container(
                    content=ft.Column([
                        ft.Text("Available Products", style=AppTextStyles.HEADER_MEDIUM),
                        self.products_grid
                    ]),
                    expand=7,
                ...
                ft.Container(
                    content=ft.Column([
                        ft.Text("Current Order", style=AppTextStyles.HEADER_MEDIUM),
                        ft.Divider(),
                        self.cart_list,
                        ft.Divider(),
                    ...
                    bgcolor=AppColors.BACKGROUND_VARIANT,
                    border_radius=10,
                )
            ],
            expand=True
    ...
    def create_product_card(self, product: Product):
        return ft.Container(
            content=ft.Column([
                ft.Icon(ft.Icons.SHOPPING_BAG, size=40, color=AppColors.PRIMARY),
                ft.Text(product.name, style=AppTextStyles.LABEL_BOLD, text_align="center"),
                ft.Text(f"${product.price:.2f}", size=14),
                ft.Text(f"Stock: {product.quantity}", size=12, color=AppColors.TEXT_SECONDARY),
                ...
                ft.Container(
                    content=ft.Row([
                        ft.Column([
                            ...
                            ft.Text(f"${product.price:.2f}", size=12, color=AppColors.TEXT_SECONDARY),
                        ], expand=True),
                    ...
                    padding=5,
                    border=ft.border.only(bottom=ft.border.BorderSide(1, AppColors.DIVIDER))
                )
            )
    ...
    def calculate_change(self, e):
        try:
            ...
            self.change_text.color = AppColors.SUCCESS if change >= 0 else AppColors.ERROR
            self.update()
        except ValueError:
            self.change_text.value = "Change: $0.00"
            self.update()
    ...
    def process_payment(self, e):
        try:
            ...
            payment_method_str = self.payment_method_dropdown.value
            payment_method = PaymentMethod(payment_method_str)

            sale = self.payment_controller.create_sale(
                items=items_to_process,
                payment_method=payment_method
            )
            
            logger.info(f"Sale successful: {sale.id}")
            ...
            self.page.show_snack_bar(ft.SnackBar(content=ft.Text("Payment successful!"), bgcolor=AppColors.SUCCESS))
            
        except ValueError as ve:
            logger.error(f"Payment validation error: {ve}")
            self.page.show_snack_bar(ft.SnackBar(content=ft.Text(str(ve)), bgcolor=AppColors.ERROR))
        except Exception as ex:
            logger.exception(f"Payment processing failed: {ex}")
            self.page.show_snack_bar(ft.SnackBar(content=ft.Text("An error occurred during payment."), bgcolor=AppColors.ERROR))
